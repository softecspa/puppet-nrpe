...
sub help() {
   print <<HERE;
    ...
    Note :  LiB for lower is better and HiB higher is better.
...
# Formatting of the data table
    ...
    # lcocchi: calcolo l'indice dell'array per passarlo a splice()
    my ( $index )= grep { $recolte[$_] =~ /^$/ } 0..$#recolte;

    # lcocchi: sostituito $topnbr con ++$index
    #  splice (@recolte, 0, $topnbr);
    splice (@recolte, 0, ++$index);

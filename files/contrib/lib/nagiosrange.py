"""
Contains a class to represent a range that adheres to the  range  format
defined by Nagios.
"""
...
class Range(object):
    ...
    def __init__(self, value):
        """
        Initializes a Nagios range  with  the  given  value.  The  value
        should be in the Nagios range format, which is the following:
        ...
        Notes:
          ...
          - Ranges by default are exclusive. A range of ``10:20`` will match
            values that are ``< 10 OR > 20``.
          - ``@`` means the range is `inclusive`. So ``@10:20`` is valid in
            the case that the value is ``>= 10 AND <= 20``.
          - If ``start`` or ``end`` is ``~``, this value is negative or
            positive inifinity, respectively. A range of ``~:20`` will match
            values that are ``> 20`` only.
          - If ``start`` is not given, then it is assumed to be 0.
          - If ``end`` is not given, but a ``:`` exists, then ``end`` is
            assumed to be infinity. Example: ``5:`` would match ``< 5``.
        """
        if type(value) != str:
            raise RangeValueError('Range: value must to be string')
        ...
        try:
            ...
            raise RangeValueError('Range: invalid start value: %s' % parts[0])
        ...
        try:
            ...
            raise RangeValueError('Range: invalid end value: %s' % parts[1])
    ...
    def in_range(self, value):
        ...
        if type(value) not in [int, float]:
            raise RangeValueError('value must to be int or float')
    ...
    def __str__(self):
        ...
        be passed to another plugin or used for debug output. The string
        returned from here should generally be equivalent to  the  value
        given to the constructor,  but  sometimes  it  can  be  slightly
